...
diagnosis. Name the variable or argument that matters and leave its value out.
...
`coredumpctl info` shows the newest entry for that PID; when it lists several,
say so.
...
reconfigure — with ONE exception: if the root cause is in this machine's own
configuration (the dotfiles repo at `~/Projects/dotfiles`), you may fix it there.
The one thing to clean up is your own: delete the core you extracted
above, which is a copy of the crashed process's memory.

## Reporting

Most crashes are upstream bugs in the crashed application, not this machine's
configuration. NEVER file, comment on, or react to upstream issues or pull
requests yourself, on GitHub or anywhere else -- summarize your findings in the
terminal instead, and Michael will file anything worth filing by hand. When the
evidence points at a known upstream issue, cite it as plain text (a code span
like `owner/repo#123`), never as an autolinking reference.
